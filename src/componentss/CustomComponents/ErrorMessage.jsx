 ...
 const ErrorMessage = ({ 
  ...
  const ErrorDefault = () => (
    <div className={`flex flex-col items-center justify-center text-center ${fullScreen ? 'min-h-screen' : 'p-8'}`}>
  ...
  const ErrorMinimal = () => (
    <div className={`flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-lg ${fullScreen ? 'm-8' : ''}`}>
  ...
  const ErrorCard = () => (
    <div className={`bg-white border border-red-200 rounded-lg shadow-sm ${fullScreen ? 'm-8' : 'p-6'} ${!fullScreen ? 'p-6' : ''}`}>
